...
| --- | --- | --- |
| users | email text255; password_hash text; real_name text120; role PLAYER/ORGANIZER; fixture_key optional text64; email_verified_at optional time; linked_wallet_address optional canonical Solana public-key text64; wallet_verified_at optional time | normalized email unique; non-null fixture key unique and permitted only for the development-only local auth catalog; wallet deliberately not unique; session determines actor; the address is a Devnet-linked Solana public key, not proof of settlement or user uniqueness |
| sessions | user_id; token_id text; authentication_mode EMAIL_VERIFIED/DEMO_BYPASS/SEED_FIXTURE; expires_at time; revoked_at optional time; csrf_hash text | token_id unique; revoke on logout/reset; DEMO_BYPASS never changes users.email_verified_at and is rejected/revoked when its staging-only flag is disabled; SEED_FIXTURE is allowed only for a catalog-owned user in development and never changes verification evidence |
